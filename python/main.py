...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['HYDICE_urban_resize']))
@click.argument('net_name', type=click.Choice(['HYDICE_urban_CTA']))
@click.argument('xp_path', type=click.Path(exists=True))
@click.argument('data_path', type=click.Path(exists=True))
@click.argument('patch_size', type=int, default=1)#, help='Size of the spatial neighbourhood.')
@click.argument('ignored_labels', type=list, default=[2])#, help='ignored_labels.')
@click.argument('flip_augmentation', type=bool, default=False)#, help='Random flips (if patch_size > 1).')
@click.argument('radiation_augmentation', type=bool, default=False)#, help='Random radiation noise (illumination).')
@click.argument('mixture_augmentation', type=bool, default=False)#, help='Random mixes between spectra.')
@click.argument('center_pixel', type=bool, default=True)#, help='center_pixel.')
@click.argument('supervision', type=str, default='full')#, help='supervision.')
...
@click.option('--pretrain', type=bool, default=True,#False,#
...
def main(dataset_name, net_name, xp_path, data_path, patch_size, ignored_labels,
         flip_augmentation, radiation_augmentation, mixture_augmentation, center_pixel, supervision,
         load_config, load_model, eta,
    ...
    deepCTA = CTA(cfg.settings['eta'])
    deepCTA.set_network(net_name)

    # If specified, load Deep SAD model (center c, network weights, and possibly autoencoder weights)
    if load_model:
        deepCTA.load_model(model_path=load_model, load_ae=True, map_location=device)
    ...
    if pretrain:
        ...
        deepCTA.save_ae_results(export_json=xp_path + '/ae_results.json')
    ...
    # Test model
    deepCTA.test(dataset, device=device, n_jobs_dataloader=n_jobs_dataloader)

    # Save results, model, and configuration
    deepCTA.save_results(export_json=xp_path + '/results.json')
    deepCTA.save_model(export_model=xp_path + '/model.tar')
